...
switch($action) {
	case 'create' :
		...
		if(!is_wp_error($new_id)) {
			...
			$templates = $this->get_templates();
			$selected_template_object = get_post($selected_template_id);
			$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'oblivion') . '<strong>' . $template_name . '</strong>' . __(' page template has been successfully created', 'oblivion') . '</p></div>';
		} else {
			$errors = '<ul>';
			...
			$errors .= '</ul>';
			$messages[] = '<div id="message" class="error"><p>' . __('Sorry, the operation was unsuccessful for the following reason(s): ', 'oblivion') . '</p>' . $errors . '</div>';
		}
		break;
	case 'update' :
		...
		$templates = $this->get_templates();
		$selected_template_object = get_post($selected_template_id);
		$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'oblivion') . '<strong>' . $template_name . '</strong>' . __(' page template has been updated', 'oblivion') . '</p></div>';
		break;
	case 'delete' :
		...
		$selected_template_object = get_post($selected_template_id);
		$messages[] = '<div id="message" class="updated"><p>' . __('The template has been successfully deleted', 'oblivion') . '</p></div>';
		break;
}
global $current_user;
	...
	<div id="page-builder-frame">
		<div id="page-builder-column" class="metabox-holder <?php echo $disabled ?>">
			<div id="page-builder-archive" class="postbox">
				<h3 class="hndle"><span><?php _e('Available Blocks', 'oblivion') ?></span><span id="removing-block"><?php _e('Deleting', 'oblivion') ?></span></h3>
				<div class="inside">
					...
					<form id="update-page-template" action="<?php echo $this->args['page_url'] ?>" method="post" enctype="multipart/form-data">
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										<label class="open-label" for="template-name">
											<span><?php _e('Template Name', 'oblivion') ?></span>
										...
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? __( 'Create Template', 'oblivion' ) : __( 'Save Template', 'oblivion' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
						...
						<div id="pb-body">
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if($selected_template_id === 0) {
									...
									<br/><br/>
									When you have finished building your custom page template, make sure you click the Save Template button.', 'oblivion');
									echo '</p>';
								} else {
						...
						<div id="pb-footer">
							<div class="major-publishing-actions cf">
								<div class="publishing-action">
									<?php if(!empty($selected_template_id)) {
										submit_button( __( 'Save Template','oblivion' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_footer' ) );
									} ?>
								</div><!-- END .publishing-action -->
							</div><!-- END .major-publishing-actions -->
						</div>
